...
export async function syncWallet(address, {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const txBatch = await fetchSenderTransactions(address, {
      afterHeight,
      maxPages: afterHeight ? 25 : 15,
      pageSize: 100,
    });
    if (afterHeight && !txBatch.complete) {
      throw new Error(`Backlog blockchain troppo ampio: letti ${txBatch.pagesRead || 0} blocchi di transazioni senza raggiungere l’ultimo punto salvato`);
    }
