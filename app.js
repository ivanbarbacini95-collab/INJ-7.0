...
/* v7.8.2 — Live Display follows the wallet selected in the dashboard */
(() => {
  ...
  function render(){
    ...
    const available=n(state?.available),staked=n(state?.staked),rewards=n(state?.rewards);
    ...
    if($id('landscapeWalletWorth')) rollValue('landscapeWalletWorth',new Intl.NumberFormat('it-IT',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2}).format(total*price),total*price);
    if($id('landscapeWalletInj')) $id('landscapeWalletInj').textContent=`${fmtInj(total)} INJ`;
    if($id('landscapeWalletAddress')) $id('landscapeWalletAddress').textContent=address?short(address):'Nessun wallet selezionato';
    if($id('landscapeWalletAvailable')) $id('landscapeWalletAvailable').textContent=fmtInj(available);
    if($id('landscapeWalletStaked')) $id('landscapeWalletStaked').textContent=fmtInj(staked);
    if($id('landscapeWalletRewards')) $id('landscapeWalletRewards').textContent=fmtInj(rewards);
  }
  async function enter(){
    ...
    render();clearInterval(timer);timer=setInterval(render,500);
  ...
  function init(){
    ...
    document.addEventListener('inj:wallet-changed',render);
