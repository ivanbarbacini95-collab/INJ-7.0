...
function buildValueRoller(element, text) {
  const value = String(text);
  const roller = document.createElement('span');
  roller.className = 'num-roller';
  ...
  [...value].forEach((char, index) => {
    ...
    if (isDigitChar(char)) {
      host.className = 'num-char num-digit';
      const current = document.createElement('span');
      current.className = 'num-digit-current';
      current.textContent = char;
      host.appendChild(current);
    } else {
      host.className = 'num-char num-static';
      host.textContent = char;
    }
    roller.appendChild(host);
  });

  element.replaceChildren(roller);
  element.setAttribute('aria-label', value);
  return roller;
}

function canRollValue(element, previousText, nextText) {
  if (!previousText || previousText === nextText || previousText.length !== nextText.length) return false;
  const roller = element.querySelector(':scope > .num-roller');
  if (!roller || roller.children.length !== previousText.length) return false;

  for (let index = 0; index < previousText.length; index++) {
    const before = previousText[index];
    const after = nextText[index];
    if (before === after) continue;
    if (!isDigitChar(before) || !isDigitChar(after)) return false;
  }
  return true;
}
...
function runValueTransition(element, nextText, nextNumeric, options = {}) {
  ...
  const duration = Math.max(360, number(options.duration) || 560);
  ...
  if (!animate || state.suppressEffects || !numericChange || !canRollValue(element, previousText, nextText)) {
  ...
  const travel = movement === 'up' ? -100 : 100;
  const easing = 'cubic-bezier(.22,.72,.22,1)';
  const roller = element.querySelector(':scope > .num-roller');
  const hosts = [...roller.children];
  const animations = [];
  const changed = [];
  ...
  for (let index = 0; index < nextText.length; index++) {
    ...
    const current = host?.querySelector(':scope > .num-digit-current');
    if (!host || !current) {
      renderStableValue(element, nextText, next);
      return;
    }
    ...
    incoming.className = `num-digit-next tone-${tone}`;
    incoming.textContent = nextText[index];
    host.appendChild(incoming);
    changed.push({ host, current, incoming, char: nextText[index] });

    animations.push(current.animate([
      { transform: 'translateY(0%)', opacity: 1 },
      { transform: `translateY(${travel}%)`, opacity: .12 }
    ], { duration, easing, fill: 'forwards' }));

    animations.push(incoming.animate([
      { transform: `translateY(${-travel}%)`, opacity: .2 },
      { transform: 'translateY(0%)', opacity: 1 }
    ], { duration, easing, fill: 'forwards' }));
  }
  ...
  Promise.allSettled(animations.map((animation) => animation.finished)).then(() => {
    ...
    changed.forEach(({ current, incoming, char }) => {
      current.textContent = char;
