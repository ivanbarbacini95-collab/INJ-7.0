...
function signalDashboardCard(id,numericValue){
  ...
  if(document.body.classList.contains('card-glow-disabled')) return;
...
const ROLLING_IDS=new Set([
  ...
  'circulatingSupplyValue','liveTotalSupply','networkTotalStaked','networkStakedPct',
  'networkInjBurned','networkBurnedPct','walletStakedShare','walletSupplyShare',
  'rewardPerSecond','growthCurrent','growthDelta','growthCompoundCount',
...
const pendingAccountSnapshots=new Map();
function stableMarketUpdate(next){
  const candidate=number(next?.price);
  if(!(candidate>0)) return null;
  const current=number(state.price);
  if(current>0){
    const ratio=candidate/current;
    // Ignore isolated malformed websocket ticks. A real move is accepted once
    // it arrives through a subsequent consistent update or a full REST reload.
    if(ratio<0.65||ratio>1.35){
      console.warn('Tick prezzo anomalo ignorato',current,candidate);
      return null;
    }
  }
  const low=number(next?.low), high=number(next?.high);
  return {
    ...next,
    price:candidate,
    change:Number.isFinite(Number(next?.change))?Number(next.change):state.change,
    low:low>0?low:state.low,
    high:high>0?high:state.high
  };
}
function stableAccountSnapshot(address,next){
  const snapshot={
    available:Math.max(0,number(next?.available)),
    staked:Math.max(0,number(next?.staked)),
    rewards:Math.max(0,number(next?.rewards))
  };
  snapshot.total=snapshot.available+snapshot.staked+snapshot.rewards;
  const previousTotal=number(state.available)+number(state.staked)+number(state.rewards);
  if(!(previousTotal>0)) return snapshot;
  const ratio=snapshot.total/previousTotal;
  const suspicious=!Number.isFinite(ratio)||ratio<0.35||ratio>3;
  const key=walletKey(address);
  if(!suspicious){ pendingAccountSnapshots.delete(key); return snapshot; }
  const signature=[snapshot.available,snapshot.staked,snapshot.rewards].map(v=>v.toFixed(6)).join('|');
  const pending=pendingAccountSnapshots.get(key);
  if(pending&&pending.signature===signature&&Date.now()-pending.time<120000){
    pendingAccountSnapshots.delete(key);
    return snapshot;
  }
  pendingAccountSnapshots.set(key,{signature,time:Date.now()});
  console.warn('Snapshot wallet anomalo in verifica',address,{previousTotal,nextTotal:snapshot.total});
  return null;
}
function updatePrice(next){
  const stable=stableMarketUpdate(next); if(!stable) return;
  Object.assign(state,stable); updateLiveMarketCandle(state.price); sampleHistory(state.priceHistory,state.price,HISTORY.priceStep,HISTORY.priceLimit); savePriceHistory(); scheduleMarketUi();
}
...
async function loadAccount(showFeedback=true){
  ...
  try{
    ...
    const accountSnapshot=stableAccountSnapshot(address,{
      available:findAmount(bank?.balances||[]),
      staked:parseDelegations(delegations),
      rewards:parseRewards(rewards)
    });
    if(!accountSnapshot){
      status('online','Verifica dati wallet…');
      setTimeout(()=>{ if(address===state.address) loadAccount(false); },1800);
      return;
    }
    state.available=accountSnapshot.available;
    state.staked=accountSnapshot.staked;
    state.rewards=accountSnapshot.rewards;
...
(()=>{
  ...
  drawer?.addEventListener('click',e=>{
    ...
    if(action==='manage-wallets'||action==='wallets'){closeDrawer();setTimeout(()=>openWalletManager(),80)}
...
(() => {
  ...
  function applyChartsHidden(hidden){
    ...
    setButton('drawerHideCharts',hidden,hidden?'Mostra grafici':'Nascondi grafici','▱');
    requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  }
  function showLive(){
...
(() => {
  ...
  function applyHideCharts(hidden){
    ...
    const control=$('drawerHideCharts');
    setControl(control, hidden, hidden ? 'Mostra grafici' : 'Nascondi grafici');
    if(control) control.title=hidden?'Mostra nuovamente tutte le sezioni con grafici':'Nascondi tutte le sezioni con grafici';
    requestAnimationFrame(() => window.dispatchEvent(new Event('resize')));
  }
...
/* v8.2 — Live Display stability: remove rolling rebuilds and layout jumps */
(() => {
  const stableLiveIds = new Set([
    'landscapeMarketPrice','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
...
/* v8.4 — independent card illumination control */
(() => {
  const STORAGE_KEY='inj_card_glow_v84';
  const button=()=>document.getElementById('drawerCardGlow');
  const read=()=>{ try { return localStorage.getItem(STORAGE_KEY)!=='off'; } catch { return true; } };
  const write=value=>{ try { localStorage.setItem(STORAGE_KEY,value?'on':'off'); } catch {} };

  function clearActiveGlow(){
    document.querySelectorAll('#dashboardView .card.live-data-up,#dashboardView .card.live-data-down,#dashboardView .card.value-glow-up,#dashboardView .card.value-glow-down')
      .forEach(card=>card.classList.remove('live-data-up','live-data-down','value-glow-up','value-glow-down'));
  }

  function apply(enabled){
    document.body.classList.toggle('card-glow-disabled',!enabled);
    const control=button();
    if(control){
      control.classList.toggle('active',enabled);
      control.setAttribute('aria-pressed',enabled?'true':'false');
      control.title=enabled?'Illuminazione card attiva — premi per disattivarla':'Illuminazione card disattivata — premi per attivarla';
      const parts=control.querySelectorAll('span');
      if(parts[0]) parts[0].textContent=enabled?'◈':'◇';
      if(parts[1]) parts[1].textContent=`Luce card: ${enabled?'ON':'OFF'}`;
    }
    if(!enabled) clearActiveGlow();
    write(enabled);
  }
  ...
  function init(){
    ...
    button()?.addEventListener('click',event=>{
      event.preventDefault();
      event.stopImmediatePropagation();
      apply(document.body.classList.contains('card-glow-disabled'));
    },true);
  }
