...
function canRollDigits(previousText, nextText) {
  if (!previousText || previousText.length !== nextText.length) return false;
  for (let i = 0; i < previousText.length; i++) {
    const a = previousText[i];
    const b = nextText[i];
    if (a === b) continue;
...
function createSettledNumberHost(text) {
  const host = document.createElement('span');
  host.className = 'number-roll-host number-roll-settled';
  for (const char of String(text)) {
    const fixed = document.createElement('span');
    fixed.className = `roll-fixed${/\d/.test(char) ? ' roll-num' : ''}`;
...
function renderSettledValue(element, text, numericValue) {
  clearTimeout(element._rollTimer);
  element.classList.remove('number-rolling');
  element.replaceChildren(createSettledNumberHost(text));
...
function runValueTransition(element, nextText, nextNumeric, options = {}) {
  ...
  if (!animate || state.suppressEffects || !numericChange || !canRollDigits(previousText, nextText)) {
    renderSettledValue(element, nextText, next);
    return;
  }
  ...
  element.dataset.rollToken = token;
  element.classList.add('number-rolling');

  const host = document.createElement('span');
  host.className = 'number-roll-host';
  ...
  for (let i = 0; i < nextText.length; i++) {
    ...
    if (oldChar === newChar) {
      const fixed = document.createElement('span');
      fixed.className = `roll-fixed${/\d/.test(newChar) ? ' roll-num' : ''}`;
    ...
    cell.className = `roll-digit roll-${movement} tone-${tone}`;
    const track = document.createElement('span');
    track.className = 'roll-track';

    const oldFace = document.createElement('span');
    oldFace.className = 'roll-face roll-old';
    oldFace.textContent = oldChar;
    const newFace = document.createElement('span');
    newFace.className = 'roll-face roll-new';
    newFace.textContent = newChar;

    if (movement === 'up') track.append(oldFace, newFace);
    else track.append(newFace, oldFace);
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => host.classList.add('roll-start')));
  element._rollTimer = setTimeout(() => {
    if (element.dataset.rollToken !== token) return;
    renderSettledValue(element, nextText, next);
...
function setValue(id, text, numericValue, options = {}) {
  ...
  const sameTarget = latestText === nextText &&
    ((!Number.isFinite(next) && !Number.isFinite(latestNumeric)) || (Number.isFinite(next) && Number.isFinite(latestNumeric) && next === latestNumeric));
  ...
  if (element.classList.contains('number-rolling')) {
