...
/* v7.7.2 — drag & drop card layout, desktop + touch */
(()=>{
  const STORAGE_KEY='inj_card_layout_v772';
  ...
  function containerKey(container,index){
    ...
    const classes=[...container.classList].filter(c=>!['active'].includes(c));
    return `zone-${safeId(classes.join('-')||container.tagName.toLowerCase())}-${index}`;
  }
  function directCards(container){
    return [...container.children].filter(el=>el.matches?.('article.card'));
  }
  function discover(){
    editableContainers.length=0;
    const candidates=[document.getElementById('dashboardView'),...document.querySelectorAll('#dashboardView section,#dashboardView .metric-grid,#dashboardView .insight-grid,#dashboardView .pro-grid')]
      .filter(Boolean);
    [...new Set(candidates)].forEach((container,index)=>{
      const cards=directCards(container);
      if(cards.length<2)return;
      const key=containerKey(container,index);
      ...
      cards.forEach((card,i)=>{
        if(!card.dataset.layoutItem){
          ...
          card.dataset.layoutItem=`${key}-${safeId(semantic).slice(0,56)}-${i}`;
        }
      });
  ...
  function saveLayout(){
    const layout={};
    editableContainers.forEach(container=>layout[container.dataset.layoutContainer]=directCards(container).map(c=>c.dataset.layoutItem));
  ...
  function restoreLayout(){
    ...
    editableContainers.forEach(container=>{
      const order=layout[container.dataset.layoutContainer];
  ...
  function banner(show){
    ...
    if(show&&!el){
      ...
      el.innerHTML='<span>↕ Trascina le card nella posizione desiderata</span><button type="button">Fatto</button>';
      el.querySelector('button').addEventListener('click',()=>setEditMode(false));
  ...
  function setEditMode(on){
    editMode=Boolean(on);document.body.classList.toggle('layout-edit-mode',editMode);banner(editMode);syncButton();
  ...
  function beginDrag(card,e){
    ...
    ghost.classList.add('layout-drag-ghost');ghost.style.width=`${rect.width}px`;ghost.style.height=`${rect.height}px`;ghost.style.left=`${rect.left}px`;ghost.style.top=`${rect.top}px`;
    document.body.appendChild(ghost);card.classList.add('layout-dragging');
    drag={card,ghost,placeholder,container:card.parentElement,offsetX:e.clientX-rect.left,offsetY:e.clientY-rect.top};
  }
  function moveDrag(e){
    if(!drag)return;
    drag.ghost.style.left=`${e.clientX-drag.offsetX}px`;drag.ghost.style.top=`${e.clientY-drag.offsetY}px`;
    const target=document.elementFromPoint(e.clientX,e.clientY)?.closest?.('article.card[data-layout-item]');
  ...
  function endDrag(){
    if(!drag)return;
    drag.placeholder.replaceWith(drag.card);drag.card.classList.remove('layout-dragging');drag.ghost.remove();drag=null;saveLayout();
  }
  function bind(){
    discover();restoreLayout();
    document.getElementById('drawerLayoutEdit')?.addEventListener('click',()=>setEditMode(!editMode));
    document.getElementById('drawerLayoutReset')?.addEventListener('click',()=>{
      try{localStorage.removeItem(STORAGE_KEY)}catch{}
      location.reload();
    });
    document.addEventListener('pointerdown',e=>{
      if(!editMode||e.button>0)return;
      const card=e.target.closest?.('article.card[data-layout-item]');if(!card)return;
      if(e.target.closest('button,input,select,textarea,a,[contenteditable="true"]'))return;
      e.preventDefault();card.setPointerCapture?.(e.pointerId);beginDrag(card,e);
    },{passive:false});
...
/* v7.7.4 — Live Display mode (desktop + mobile landscape) */
(()=>{
  let active=false, syncTimer=null, observer=null;
  const $id=id=>document.getElementById(id);
  const sourceIds=['marketPrice','marketChange','netWorthUsd','netWorthInj'];
  function copyText(source,target){const s=$id(source),t=$id(target);if(t)t.textContent=s?.textContent?.trim()||'—';}
  function syncValues(){
    copyText('marketPrice','landscapeMarketPrice');
    copyText('marketChange','landscapeMarketChange');
    copyText('netWorthUsd','landscapeNetWorthUsd');
    copyText('netWorthInj','landscapeNetWorthInj');
  ...
  async function enter(){
    ...
    const btn=$id('drawerLandscapeLive');btn?.classList.add('active');btn?.setAttribute('aria-pressed','true');
    $id('drawerClose')?.click();syncValues();
    ...
    if(isMobileLike){try{await screen.orientation?.lock?.('landscape');}catch{}}
  }
  async function exit(){
  ...
  function bind(){
    $id('drawerLandscapeLive')?.addEventListener('click',()=>active?exit():enter());
    $id('landscapeLiveExit')?.addEventListener('click',exit);
    document.addEventListener('keydown',e=>{if(active&&e.key==='Escape')exit();});
    document.addEventListener('fullscreenchange',()=>{if(active&&!document.fullscreenElement)syncValues();});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();
})();
