...
function signalChange(element, previous, next, { cardFlash = true, positiveWhen = 'up', minInterval = 420 } = {}) {
  ...
  // Il valore viene aggiornato sempre, ma l'animazione e' limitata per evitare
  // ripartenze continue sui dati live (es. prezzo INJ e controvalori fiat).
  if (now - number(element.dataset.lastValueAnimation) >= minInterval) {
    ...
    element.classList.add(`value-change-${tone}`);
  }

  // Il glow della card e' indipendente dallo scorrimento del numero.
  // Alcune card con valori derivati live (Reward Tracker) lo disabilitano apposta.
...
function setValue(id, text, numericValue, options = {}) {
  ...
  // Compatibilita' con la vecchia opzione flash:false: ora significa solo
  // nessun glow di card, mentre il numero puo' comunque animarsi.
  ...
  // Un dato cambia visivamente solo se cambia davvero cio' che e' mostrato.
  // I decimali interni invisibili non generano ne' glow ne' animazioni.
  ...
  if (animate && !state.suppressEffects && visibleChange && numericChange) {
    signalChange(element, previous, next, { cardFlash, positiveWhen, minInterval });
  }
...
function setLoading(loading) {
  ...
  document.body.classList.toggle('wallet-loading', loading);
