...
function normalizeTheme(value){return THEME_ORDER.includes(value)?value:'black'}
function currentTheme(){
  if(document.body.classList.contains('light')) return 'light';
  if(document.body.classList.contains('ocean')) return 'ocean';
  return 'black';
}
function themeMetaColor(theme){return theme==='light'?'#eef2f7':theme==='ocean'?'#071727':'#000000'}
function applyTheme(theme,{persist=true,redraw=true}={}){
  const next=normalizeTheme(theme);
  document.body.classList.toggle('light',next==='light');
  document.body.classList.toggle('ocean',next==='ocean');
  document.body.dataset.theme=next;
  if(persist) storage.set('inj_theme',next);
  const meta=document.querySelector('meta[name="theme-color"]');
  if(meta) meta.setAttribute('content',themeMetaColor(next));
  const headerButton=$('themeButton');
  if(headerButton){
    headerButton.setAttribute('aria-pressed',next!=='black'?'true':'false');
    headerButton.setAttribute('aria-label',`Tema ${next==='black'?'nero':next==='light'?'chiaro':'blu scuro'}`);
  }
  document.dispatchEvent(new CustomEvent('inj-theme-change',{detail:{theme:next}}));
  if(redraw&&typeof drawAll==='function') requestAnimationFrame(()=>drawAll());
  return next;
}
function cycleTheme(){
  const index=THEME_ORDER.indexOf(currentTheme());
  return applyTheme(THEME_ORDER[(index+1)%THEME_ORDER.length]);
}
...
function init(){
  ...
  applyTheme(storage.get('inj_theme','black'),{persist:false,redraw:false});
  if(storage.get('inj_data_mode_v56','off')==='on') document.body.classList.add('data-mode');
  $('themeButton')?.setAttribute('aria-pressed',currentTheme()!=='black'?'true':'false');
...
(() => {
  ...
  function pulseValue(el,oldText,newText){
    ...
    if(el.id==='syncCountdown' || el.closest('.price-simulator-card') || el.closest('#landscapeLive')) return;
...
(()=>{
  ...
  function syncInterfaceModes(){
    ...
    Object.entries(modeButtons).forEach(([key,button])=>{
      ...
      const active=isTheme?states[key]!=='black':Boolean(states[key]);
      button.classList.toggle('active',active);
      button.setAttribute('aria-pressed',active?'true':'false');
      if(isTheme){
        const theme=states[key];
        button.querySelector('.interface-icon').textContent=theme==='light'?'☀':theme==='ocean'?'≈':'●';
        const label=button.querySelector('span:last-child');
        if(label) label.textContent=theme==='light'?'Chiaro':theme==='ocean'?'Blu scuro':'Nero';
        button.title='Cambia tema: Nero, Chiaro o Blu scuro';
      }
  ...
  applyTheme(localStorage.getItem('inj_theme')||'black',{persist:false,redraw:false});
  document.body.classList.toggle('reduce-motion',!motionEnabled);
  document.addEventListener('inj-theme-change',syncInterfaceModes);
  syncInterfaceModes();
})();
...
/* v7.7.7 — Live Display wallet selector: single wallet or all, kept separated */
(() => {
  const $id=id=>document.getElementById(id);
  const key=a=>String(a||'').trim().toLowerCase();
  const short=a=>{const s=String(a||'');return s.length>18?`${s.slice(0,9)}…${s.slice(-6)}`:s||'Wallet'};
  const n=v=>Number(v)||0;
  const fmtUsd=v=>new Intl.NumberFormat('it-IT',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2}).format(n(v));
  const fmtInj=v=>new Intl.NumberFormat('it-IT',{minimumFractionDigits:4,maximumFractionDigits:6}).format(n(v));
  let open=false,timer=0;

  function wallets(){
    const list=[...(state?.wallets||[])];
    if(state?.address && !list.some(a=>key(a)===key(state.address))) list.unshift(state.address);
    return [...new Map(list.filter(Boolean).map(a=>[key(a),a])).values()];
  }
  function walletData(address){
    const k=key(address),active=k===key(state?.address);
    if(active) return {address,available:n(state.available),staked:n(state.staked),rewards:n(state.rewards),loaded:true,active:true};
    const c=state?.walletCache?.[k]||{};
    return {address,available:n(c.available),staked:n(c.staked),rewards:n(c.rewards),loaded:Boolean(c.updated||c.total),active:false};
  }
  function updateSelect(){
    const select=$id('liveDisplayWalletSelect'); if(!select)return;
    const previous=select.value||localStorage.getItem('inj_live_display_wallet_v777')||key(state?.address)||'';
    const rows=wallets();
    select.innerHTML=rows.map(a=>`<option value="${key(a)}">${short(a)}</option>`).join('');
    select.value=rows.some(a=>key(a)===previous)?previous:(key(state?.address)||key(rows[0])||'');
  }
  function render(){
    if(!open)return;
    const price=n(state?.price),change=n(state?.change);
    const priceEl=$id('landscapeMarketPrice'),changeEl=$id('landscapeMarketChange');
    if(priceEl) rollValue('landscapeMarketPrice',`$${price.toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4})}`,price);
    if(changeEl){changeEl.textContent=`${change>=0?'+':''}${change.toFixed(2)}% · 24H`;changeEl.style.color=change>=0?'#22d3a6':'#fb7185'}
    updateSelect();
    const select=$id('liveDisplayWalletSelect'),grid=$id('liveDisplayWalletGrid'); if(!grid)return;
    const chosen=select?.value||key(state?.address)||'';
    const all=wallets().map(walletData);
    const visible=all.filter(w=>key(w.address)===chosen).slice(0,1);
    grid.dataset.mode='single';
    $id('landscapeWalletLabel').textContent='WALLET SELEZIONATO';
    if(!visible.length){grid.innerHTML='<div class="live-wallet-empty">Aggiungi almeno un wallet alla dashboard.</div>';return;}
    grid.innerHTML=visible.map(w=>{
      const total=w.available+w.staked+w.rewards;
      return `<article class="live-wallet-card ${w.active?'active':''}">
        <div class="live-wallet-card-head"><strong title="${w.address}">${short(w.address)}</strong><small>${w.active?'ATTIVO':w.loaded?'AGGIORNATO':'IN CARICAMENTO'}</small></div>
        <strong class="live-wallet-usd">${fmtUsd(total*price)}</strong>
        <span class="live-wallet-inj">${fmtInj(total)} INJ</span>
        <div class="live-wallet-breakdown"><span>Disponibili ${fmtInj(w.available)}</span><span>Stake ${fmtInj(w.staked)}</span><span>Reward ${fmtInj(w.rewards)}</span></div>
      </article>`;
    }).join('');
  }
  async function enter(){
    open=true;const host=$id('landscapeLive');if(!host)return;
    host.hidden=false;host.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';
    $id('drawerLandscapeLive')?.setAttribute('aria-pressed','true');
    updateSelect();render();clearInterval(timer);timer=setInterval(render,500);
    try{if(window.innerWidth>700&&!document.fullscreenElement)await host.requestFullscreen?.()}catch{}
  }
  async function exit(){
    open=false;clearInterval(timer);timer=0;const host=$id('landscapeLive');if(host){host.hidden=true;host.setAttribute('aria-hidden','true')}
    document.body.style.removeProperty('overflow');$id('drawerLandscapeLive')?.setAttribute('aria-pressed','false');
    try{if(document.fullscreenElement)await document.exitFullscreen?.()}catch{}
  }
  function init(){
    $id('drawerLandscapeLive')?.addEventListener('click',()=>open?exit():enter());
    $id('landscapeLiveExit')?.addEventListener('click',exit);
    $id('liveDisplayWalletSelect')?.addEventListener('change',e=>{localStorage.setItem('inj_live_display_wallet_v777',e.target.value);render()});
    document.addEventListener('fullscreenchange',()=>{if(open&&!document.fullscreenElement&&window.innerWidth>700)exit()});
    document.addEventListener('inj:wallet-changed',event=>{
      updateSelect();
      const selected=$id('liveDisplayWalletSelect');
      const activeKey=key(event?.detail?.address||state?.address);
      if(selected&&activeKey){
        selected.value=activeKey;
        localStorage.setItem('inj_live_display_wallet_v777',activeKey);
      }
      render();
    });
