...
const DIGIT_ANIMATION_IDS = new Set([
  'aggregateNetWorth', 'aggregateTotalInj', 'aggregateStaked', 'aggregateRewards',
  'marketPrice', 'dayLow', 'dayHigh', 'marketChange', 'marketChangeAmount',
  'netWorthUsd', 'netWorthInj', 'availableInj', 'availableUsd', 'stakedInj', 'stakedUsd',
  'rewardsInj', 'rewardsUsd', 'aprValue',
  'hourlyEstimate', 'hourlyUsd', 'dailyEstimate', 'dailyUsd', 'weeklyEstimate', 'weeklyUsd',
  'monthlyEstimate', 'monthlyUsd', 'yearlyEstimate', 'yearlyUsd',
  'targetMissing', 'targetPercent', 'targetPercentBadge'
]);

function canDigitAnimateText(previousText, nextText) {
  const a = Array.from(String(previousText));
  const b = Array.from(String(nextText));
  if (!a.length || a.length !== b.length || b.length > 42) return false;
  let changed = 0;
  ...
  return changed > 0 && changed <= 10;
}

function buildDigitMarkup(element, text) {
  element.replaceChildren();
  element.classList.add('digitized-value');
  Array.from(String(text)).forEach((char, index) => {
    if (/\d/.test(char)) {
      const slot = document.createElement('span');
      slot.className = 'digit-cell';
      slot.dataset.index = String(index);
      slot.dataset.value = char;

      const spacer = document.createElement('span');
      spacer.className = 'digit-cell-spacer';
      spacer.textContent = '0';

      const current = document.createElement('span');
      current.className = 'digit-cell-current';
      current.textContent = char;

      slot.append(spacer, current);
      element.appendChild(slot);
    } else {
      const fixed = document.createElement('span');
      fixed.className = 'digit-fixed-char';
      fixed.dataset.index = String(index);
...
function finalizeDigitCells(element) {
  if (!element?.classList.contains('digitized-value')) return;
  element.querySelectorAll(':scope > .digit-cell').forEach((slot) => {
    const current = slot.querySelector(':scope > .digit-cell-current');
    const next = slot.querySelector(':scope > .digit-cell-next');
    if (current) {
      current.getAnimations?.().forEach((animation) => animation.cancel());
      current.textContent = slot.dataset.value || current.textContent;
      current.style.transform = 'translateY(0)';
      current.style.removeProperty('color');
    }
    next?.getAnimations?.().forEach((animation) => animation.cancel());
    next?.remove();
    slot.classList.remove('rolling-up', 'rolling-down', 'rolling-active');
  });
}

function setDigitMarkupInstant(element, text) {
  const chars = Array.from(String(text));
  const children = Array.from(element.children);
  if (!element.classList.contains('digitized-value') || children.length !== chars.length) {
    buildDigitMarkup(element, text);
    return;
  }
  ...
  for (let index = 0; index < chars.length; index += 1) {
    ...
    if (/\d/.test(char) && child.classList.contains('digit-cell')) {
      child.dataset.value = char;
      const current = child.querySelector(':scope > .digit-cell-current');
      if (current) current.textContent = char;
    } else if (!/\d/.test(char) && child.classList.contains('digit-fixed-char')) {
...
function animateChangedDigits(element, previousText, nextText, movement, tone, minInterval = 500) {
  if (!element || document.visibilityState === 'hidden') return false;
  if (window.matchMedia?.('(prefers-reduced-motion: reduce)').matches) return false;
  if (!DIGIT_ANIMATION_IDS.has(element.id) || !canDigitAnimateText(previousText, nextText)) return false;

  const now = Date.now();
  if (now - number(element._lastDigitRollAt) < minInterval) return false;
  element._lastDigitRollAt = now;

  // Persistent in-flow character cells keep every baseline and width unchanged.
  // Only the digit inside an already-sized cell moves.
  finalizeDigitCells(element);
  if (!element.classList.contains('digitized-value')) buildDigitMarkup(element, previousText);

  const previousChars = Array.from(String(previousText));
  const nextChars = Array.from(String(nextText));
  const cells = Array.from(element.children);
  if (cells.length !== nextChars.length) return false;

  let animated = false;
  const completions = [];
  element._digitBusy = true;
  for (let index = 0; index < nextChars.length; index += 1) {
    if (previousChars[index] === nextChars[index]) continue;
    const slot = cells[index];
    if (!slot?.classList.contains('digit-cell')) return false;

    const current = slot.querySelector(':scope > .digit-cell-current');
    if (!current) return false;

    slot.dataset.value = nextChars[index];
    slot.style.setProperty('--digit-tone', tone === 'up' ? 'var(--up)' : 'var(--down)');
    slot.classList.add('rolling-active', movement === 'up' ? 'rolling-up' : 'rolling-down');

    const incoming = document.createElement('span');
    incoming.className = 'digit-cell-next';
    incoming.textContent = nextChars[index];
    slot.appendChild(incoming);

    const distance = movement === 'up' ? -1 : 1;
    const incomingStart = movement === 'up' ? 1 : -1;
    const duration = 440;
    const easing = 'cubic-bezier(.2,.72,.28,1)';

    const oldAnimation = current.animate([
      { transform: 'translateY(0)', opacity: 1 },
      { transform: `translateY(${distance * 86}%)`, opacity: .16 }
    ], { duration, easing, fill: 'forwards' });

    const newAnimation = incoming.animate([
      { transform: `translateY(${incomingStart * 86}%)`, opacity: .2 },
      { transform: 'translateY(0)', opacity: 1 }
    ], { duration, easing, fill: 'forwards' });

    animated = true;
    const completion = newAnimation.finished.then(() => {
      // Swap at the exact same pixel position reached by the incoming digit.
      // Both operations happen in one task, so there is no visible snap frame.
      current.getAnimations?.().forEach((animation) => animation.cancel());
      current.textContent = slot.dataset.value || nextChars[index];
      current.style.transform = 'translateY(0)';
      current.style.color = 'var(--digit-tone)';
      incoming.remove();
      slot.classList.remove('rolling-up', 'rolling-down', 'rolling-active');
      requestAnimationFrame(() => {
        current.style.transition = 'color .32s ease';
        current.style.removeProperty('color');
        setTimeout(() => current.style.removeProperty('transition'), 360);
      });
    }).catch(() => {});
    completions.push(completion);
    oldAnimation.finished.catch(() => {});
  }

  if (!animated) {
    element._digitBusy = false;
    return false;
  }

  Promise.allSettled(completions).then(() => {
    element._digitBusy = false;
    const queued = element._queuedDigitValue;
    element._queuedDigitValue = null;
    if (queued && queued.text !== element.dataset.displayedValue) {
      setValue(element.id, queued.text, queued.numericValue, queued.options);
    }
  });
  return true;
}
...
function setValue(id, text, numericValue, options = {}) {
  ...
  // While a digit is physically moving, keep only the newest incoming value.
  // This prevents a websocket tick from forcing the active digit to snap midway.
  if (element._digitBusy && textChanged) {
    element._queuedDigitValue = { text: nextText, numericValue, options: { ...options } };
    return;
  }

  const movement = Number.isFinite(previousNumeric) && Number.isFinite(next) && next < previousNumeric ? 'down' : 'up';
  ...
  let rolled = false;
  if (textChanged && options.animate !== false && !state.suppressEffects && numericChanged) {
    rolled = animateChangedDigits(element, previousText, nextText, movement, tone, minInterval);
  }

  if (textChanged && !rolled) {
    finalizeDigitCells(element);
    if (DIGIT_ANIMATION_IDS.has(id) && /^.*\d.*$/.test(nextText) && nextText.length <= 42) {
      setDigitMarkupInstant(element, nextText);
    } else {
      element.classList.remove('digitized-value');
  ...
  if (!rolled) pulseValue(element, tone, minInterval);
...
function renderPortfolio() {
  ...
  setValue('netWorthUsd', state.address ? money(netWorth) : '—', state.address ? netWorth : NaN, { cardFlash: false });
  setValue('netWorthInj', formatInj(totalInj, 4), totalInj);
  setValue('availableInj', formatInj(state.available), state.available);
  setValue('availableUsd', money(state.available * state.price), state.available * state.price, { cardFlash: false });
  setValue('stakedInj', formatInj(state.staked), state.staked);
  setValue('stakedUsd', money(state.staked * state.price), state.staked * state.price, { cardFlash: false });
  setValue('rewardsInj', formatInj(state.rewards), state.rewards);
  setValue('rewardsUsd', money(state.rewards * state.price), state.rewards * state.price, { cardFlash: false });
...
function renderRewardTracker() {
  ...
  setValue('hourlyUsd', active ? money(hourly * state.price, 4) : '—', active ? hourly * state.price : NaN, { cardFlash: false, minInterval: 1400 });
  setValue('dailyEstimate', active ? formatInj(daily, 7) : '—', active ? daily : NaN, { cardFlash: false, minInterval: 900 });
  setValue('dailyUsd', active ? money(daily * state.price, 3) : '—', active ? daily * state.price : NaN, { cardFlash: false, minInterval: 1400 });
  setValue('weeklyEstimate', active ? formatInj(weekly, 6) : '—', active ? weekly : NaN, { cardFlash: false, minInterval: 900 });
  setValue('weeklyUsd', active ? money(weekly * state.price, 3) : '—', active ? weekly * state.price : NaN, { cardFlash: false, minInterval: 1400 });
  setValue('monthlyEstimate', active ? formatInj(monthly, 6) : '—', active ? monthly : NaN, { cardFlash: false, minInterval: 900 });
  setValue('monthlyUsd', active ? money(monthly * state.price) : '—', active ? monthly * state.price : NaN, { cardFlash: false, minInterval: 1400 });
  setValue('yearlyEstimate', active ? formatInj(yearly, 5) : '—', active ? yearly : NaN, { cardFlash: false, minInterval: 900 });
  setValue('yearlyUsd', active ? money(yearly * state.price) : '—', active ? yearly * state.price : NaN, { cardFlash: false, minInterval: 1400 });
