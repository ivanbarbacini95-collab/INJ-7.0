...
/* v7.6.8 — Resilient multi-endpoint Injective Network Health */
(() => {
  const healthState={running:false,timer:0,lastCheck:0,results:null};
  const HEALTH_TIMEOUT=9000;
  ...
  function updateDiagnostic(text){ const node=document.getElementById('diagnosticOutput'); if(node) node.textContent=text; }
  function shortHost(url){ try{return new URL(url).hostname.replace(/^sentry\./,'sentry.')}catch{return url} }
  async function probe(url,label){
    const started=performance.now();
    try{
      await json(url,HEALTH_TIMEOUT);
      return {label,url,ok:true,latency:Math.round(performance.now()-started)};
    }catch(error){ return {label,url,ok:false,latency:0,error:String(error?.message||error)}; }
  }
  async function probeAll(){
    const wallet=String(state?.address||'').trim();
    const lcdChecks=ENDPOINTS.map((base,i)=>probe(`${base}/cosmos/base/tendermint/v1beta1/node_info`,`LCD ${i+1}`));
    const indexerPath=validAddress?.(wallet)?`/accountTxs/${encodeURIComponent(wallet)}?limit=1`:'/accountTxs?limit=1';
    const indexerChecks=EXPLORER_ENDPOINTS.map((base,i)=>probe(`${base}${indexerPath}`,`Indexer ${i+1}`));
    const results=await Promise.allSettled([...lcdChecks,...indexerChecks]);
    const normalized=results.map((r,i)=>r.status==='fulfilled'?r.value:{label:i<ENDPOINTS.length?`LCD ${i+1}`:`Indexer ${i-ENDPOINTS.length+1}`,ok:false,error:String(r.reason||'Errore')});
    return {lcd:normalized.slice(0,ENDPOINTS.length),indexer:normalized.slice(ENDPOINTS.length)};
  }
  function renderServices(groups){
    const host=document.getElementById('chainHealthServices'); if(!host) return;
    const all=[...groups.lcd,...groups.indexer];
    host.innerHTML=all.map(item=>`<div class="chain-service ${item.ok?'ok':'fail'}"><span><i></i>${item.label}</span><strong>${item.ok?`${item.latency} ms`:'fallback'}</strong><small>${shortHost(item.url||'')}</small></div>`).join('');
  }
  function renderScore(score,lcdOk,indexerOk){
    const scoreNode=document.getElementById('chainHealthScore');
    const bar=document.getElementById('chainHealthBar');
    const summary=document.getElementById('chainHealthSummary');
    if(scoreNode) scoreNode.textContent=`${score}%`;
    if(bar) bar.style.width=`${score}%`;
    if(summary) summary.textContent=`LCD ${lcdOk}/${ENDPOINTS.length} · Indexer ${indexerOk}/${EXPLORER_ENDPOINTS.length}`;
  }
  async function checkChainHealth({notify=false,syncRewards=false}={}){
    if(healthState.running) return healthState.results;
    healthState.running=true;
    updateBadge('loading','CHAIN…');
    updateDiagnostic('Controllo parallelo dei nodi Injective…');
    try{
      if(navigator.onLine===false) throw new Error('BROWSER_OFFLINE');
      const groups=await probeAll();
      healthState.results=groups; healthState.lastCheck=Date.now();
      const lcdOk=groups.lcd.filter(x=>x.ok).length;
      const indexerOk=groups.indexer.filter(x=>x.ok).length;
      const total=groups.lcd.length+groups.indexer.length;
      const active=lcdOk+indexerOk;
      const score=Math.round(active/Math.max(1,total)*100);
      renderServices(groups); renderScore(score,lcdOk,indexerOk);

      // Operational ON-CHAIN requires redundancy category coverage, not every public endpoint.
      if(lcdOk>0 && indexerOk>0){
        updateBadge('online','ON-CHAIN');
        updateDiagnostic(`Injective operativa · ${active}/${total} endpoint attivi · fallback automatico abilitato`);
        ...
        if(notify&&typeof toast==='function') toast(`ON-CHAIN · rete operativa (${score}%)`);
      }else if(lcdOk>0 || indexerOk>0){
        updateBadge('partial','RETE LIMITATA');
        updateDiagnostic(`Un solo tipo di servizio disponibile · LCD ${lcdOk}/${ENDPOINTS.length} · Indexer ${indexerOk}/${EXPLORER_ENDPOINTS.length}`);
        if(notify&&typeof toast==='function') toast('Rete Injective limitata: fallback incompleto');
      }else{
        updateBadge('error','CHAIN OFFLINE');
        updateDiagnostic('Nessun endpoint Injective raggiungibile · dati locali ancora disponibili');
        if(notify&&typeof toast==='function') toast('Servizi Injective non raggiungibili');
      }
      return groups;
    }catch(error){
      if(String(error?.message||error).includes('BROWSER_OFFLINE')){
        updateBadge('error','BROWSER OFFLINE');
        updateDiagnostic('Il dispositivo non è connesso a Internet · cache locale disponibile');
      }else{
        updateBadge('error','CHAIN OFFLINE'); updateDiagnostic(String(error?.message||error));
      }
      return null;
  ...
  const init=()=>{
    ...
    if(button){
      ...
      clone.textContent='Controlla rete e sincronizza';
    ...
    window.addEventListener('offline',()=>{updateBadge('error','BROWSER OFFLINE');updateDiagnostic('Il dispositivo non è connesso a Internet · cache locale disponibile');});
