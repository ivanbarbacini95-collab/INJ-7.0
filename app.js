...
function canAnimateDigits(previousText, nextText) {
  if (!previousText || previousText === nextText || previousText.length !== nextText.length) return false;
  for (let index = 0; index < previousText.length; index++) {
    const previous = previousText[index];
    const next = nextText[index];
    if (previous === next) continue;
    if (!isDigitChar(previous) || !isDigitChar(next)) return false;
  }
  return true;
}

function buildDigitValue(element, text) {
  const value = String(text);
  const fragment = document.createDocumentFragment();
  [...value].forEach((char, index) => {
    const host = document.createElement('span');
    host.className = isDigitChar(char) ? 'value-char digit-slot' : 'value-char value-static-char';
...
function renderStableValue(element, text, numericValue) {
  ...
  if (element._valueAnimation?.animations) {
    element._valueAnimation.animations.forEach((animation) => {
      try { animation.cancel(); } catch (_) {}
    });
  }
  element._valueAnimation = null;
  element._pendingValue = null;
  buildDigitValue(element, text);
...
function runValueTransition(element, nextText, nextNumeric, options = {}) {
  ...
  const duration = Math.max(480, number(options.duration) || 760);
  ...
  if (!animate || state.suppressEffects || !numericChange || !canAnimateDigits(previousText, nextText)) {
  ...
  const direction = movement === 'up' ? -1 : 1;
  const easing = 'cubic-bezier(.2,.74,.22,1)';
  const charHosts = [...element.querySelectorAll('.value-char')];

  if (charHosts.length !== previousText.length) {
    renderStableValue(element, previousText, previousNumeric);
  }

  const hosts = [...element.querySelectorAll('.value-char')];
  const animations = [];
  const incomingNodes = [];
  let changedDigits = 0;

  for (let index = 0; index < nextText.length; index++) {
    if (previousText[index] === nextText[index]) continue;
    const host = hosts[index];
    const current = host?.querySelector('.digit-current');
    if (!host || !current || !isDigitChar(previousText[index]) || !isDigitChar(nextText[index])) {
      renderStableValue(element, nextText, next);
      return;
    }
    ...
    incoming.className = `digit-next tone-${tone}`;
    incoming.textContent = nextText[index];
    host.appendChild(incoming);
    incomingNodes.push(incoming);

    const currentAnimation = current.animate([
      { transform: 'translate3d(0,0,0)', opacity: 1 },
      { transform: `translate3d(0,${direction * 105}%,0)`, opacity: .18 }
    ], { duration, easing, fill: 'forwards' });
    const incomingAnimation = incoming.animate([
      { transform: `translate3d(0,${-direction * 105}%,0)`, opacity: .22 },
      { transform: 'translate3d(0,0,0)', opacity: 1 }
    ], { duration, easing, fill: 'forwards' });
    animations.push(currentAnimation, incomingAnimation);
  }
  ...
  const token = {};
  element._valueAnimation = { animations, token };

  Promise.allSettled(animations.map((animation) => animation.finished)).then(() => {
    if (element._valueAnimation?.token !== token) return;
    const finalHosts = [...element.querySelectorAll('.value-char')];
    for (let index = 0; index < nextText.length; index++) {
      const host = finalHosts[index];
      const current = host?.querySelector('.digit-current');
      if (current && isDigitChar(nextText[index])) {
        current.textContent = nextText[index];
        current.style.removeProperty('transform');
        current.style.removeProperty('opacity');
      }
      host?.querySelectorAll('.digit-next').forEach((node) => node.remove());
    }
    animations.forEach((animation) => { try { animation.cancel(); } catch (_) {} });
    element._valueAnimation = null;
    element.dataset.displayedValue = nextText;
    if (Number.isFinite(next)) element.dataset.displayedNumeric = String(next);
    else delete element.dataset.displayedNumeric;
