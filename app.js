...
function setValue(id, text, numericValue) {
  const element = $(id);
  if (!element) return;
...
function setLoading(loading) {
  ...
  $('loadButton').textContent = loading ? 'Lettura…' : 'Analizza';
}

async function loadWallet(showFeedback = true) {
  const address = String($('addressInput').value || '').trim().toLowerCase();
...
function renderMarket() {
  ...
  const previous = Number(change.dataset.numericValue);
  ...
  if (state.price > 0) {
    signalChange(change, previous, next);
...
function renderPortfolio() {
  ...
  setValue('netWorthUsd', state.address ? money(netWorth) : '—', state.address ? netWorth : NaN);
  ...
  setValue('dailyUsd', state.personalApr > 0 ? money(dailyInj * state.price, 3) : '—', state.personalApr > 0 ? dailyInj * state.price : NaN);
  setValue('monthlyEstimate', state.personalApr > 0 ? formatInj(monthlyInj, 6) : '—', state.personalApr > 0 ? monthlyInj : NaN);
  setValue('monthlyUsd', state.personalApr > 0 ? money(monthlyInj * state.price) : '—', state.personalApr > 0 ? monthlyInj * state.price : NaN);
  setValue('yearlyEstimate', state.personalApr > 0 ? formatInj(yearlyInj, 5) : '—', state.personalApr > 0 ? yearlyInj : NaN);
  setValue('yearlyUsd', state.personalApr > 0 ? money(yearlyInj * state.price) : '—', state.personalApr > 0 ? yearlyInj * state.price : NaN);
...
async function refreshAll(showFeedback = true) {
  await Promise.all([
    loadMarket(),
    state.address ? loadWallet(false) : Promise.resolve()
  ]);
...
function toggleTheme() {
  ...
  localStorage.setItem('inj_monitor_theme', next);
}
...
async function init() {
  try {
    ...
    $('currencyButton').textContent = state.currency === 'EUR' ? '€' : '$';
    if (localStorage.getItem('inj_monitor_privacy') === '1') document.body.classList.add('privacy-on');
    const savedAddress = localStorage.getItem('inj_monitor_address') || '';
    if (validAddress(savedAddress)) {
  ...
  await loadMarket();
  connectMarketSocket();
  if (state.address) await loadWallet(false);

  setInterval(loadMarket, 60_000);
  setInterval(() => {
    if (state.address && !document.hidden && !state.loading) loadWallet(false);
  }, 30_000);
}
