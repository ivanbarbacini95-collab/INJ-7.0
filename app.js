...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    const syncRewardHistoryAfterOffline=()=>{
      ...
      setTimeout(syncRewardHistoryForAllWallets,250);
    };
...
/* v7.5.1 — persistent cloud history, cron catch-up and cross-device merge */
(() => {
  const CLOUD_ENDPOINT = '/api/cloud';
  const GROWTH_PREFIX = 'inj_growth_v733_';
  const CLOUD_LAST_PUSH_PREFIX = 'inj_cloud_last_push_v750_';
  const cloudState = { inFlight: false, timer: 0, activeAddress: '', lastSuccess: 0 };
  ...
  const cloudPushKey = (address = state?.address) => `${CLOUD_LAST_PUSH_PREFIX}${addressKey(address)}`;
  ...
  function setCloudStatus(mode, text) {
    ...
    const chartStatus = document.getElementById('historyCloudStatus');
    if (chartStatus) chartStatus.textContent = text;
  }
  ...
  function normalizeReward(row) {
    ...
    const validator = String(row?.validator || '');
    return {
      id: String(row?.id || (hash ? `${hash}:${validator || amount}` : `${timestamp}:${amount}:${validator}`)),
      hash,
      timestamp,
      t: Date.parse(timestamp) || Number(row?.t) || Date.now(),
      amount,
      validator,
  ...
  function mergeRewardRows(...groups) {
    const map = new Map();
    groups.flat().map(normalizeReward).filter(row => row.amount > 0 && row.t > 0).forEach(row => {
      const previous = map.get(row.id);
      if (!previous || row.amount > previous.amount) map.set(row.id, row);
    });
    return [...map.values()].sort((a, b) => b.t - a.t).slice(0, 5000);
  }

  function normalizeGrowth(row) {
    const t = Number(row?.t) || Date.parse(row?.timestamp || '') || Date.now();
    const staked = Number(row?.staked || 0);
    const delta = Number(row?.delta ?? row?.amount ?? 0);
    const type = String(row?.type || row?.kind || 'stake');
    const hash = String(row?.hash || '');
    return {
      id: String(row?.id || (hash ? `${hash}:${type}:${t}` : `${t}:${staked}:${type}`)),
      t,
      timestamp: row?.timestamp || new Date(t).toISOString(),
      staked,
      delta,
      amount: Number(row?.amount || Math.abs(delta)),
      type,
      hash,
      height: String(row?.height || ''),
  ...
  function mergeGrowthRows(...groups) {
    const map = new Map();
    groups.flat().map(normalizeGrowth).filter(row => row.t > 0 && Number.isFinite(row.staked)).forEach(row => map.set(row.id, row));
    const ordered = [...map.values()].sort((a, b) => a.t - b.t);
    const collapsed = [];
    for (const row of ordered) {
      const index = collapsed.findIndex(previous => Math.abs(previous.t - row.t) <= 5 * 60 * 1000 && Math.abs(previous.staked - row.staked) <= 0.000001);
      if (index < 0) { collapsed.push(row); continue; }
    ...
    return collapsed.sort((a, b) => a.t - b.t).slice(-10000);
  }

  function localPayload(address = state.address) {
    return {
      address: addressKey(address),
      rewardHistory: mergeRewardRows(
        typeof savedRewardHistory === 'function' ? savedRewardHistory(address) : [],
        addressKey(address) === addressKey(state.address) ? state.rewardHistory || [] : [],
      ),
      stakingGrowth: mergeGrowthRows(jsonRead(growthStorageKey(address), [])),
    };
  }

  function applyCloudData(data, address = state.address) {
    if (!data) return;
    const wallet = addressKey(address);
    const isActive = wallet === addressKey(state.address);
    const rewards = mergeRewardRows(
      typeof savedRewardHistory === 'function' ? savedRewardHistory(address) : [],
      isActive ? state.rewardHistory || [] : [],
      data.rewardHistory || [],
    );
    if (typeof storage?.setJSON === 'function' && typeof rewardHistoryStorageKey === 'function') {
      storage.setJSON(rewardHistoryStorageKey(address), rewards);
      storage.set(rewardHistoryStorageKey(address) + ':lastSync', String(Date.now()));
    }

    const growth = mergeGrowthRows(jsonRead(growthStorageKey(address), []), data.stakingGrowth || []);
    jsonWrite(growthStorageKey(address), growth);

    if (isActive) {
      state.rewardHistory = rewards;
      state.rewardHistoryLoaded = true;
      state.rewardHistoryLastSync = Date.now();
      if (typeof renderRewardHistory === 'function') renderRewardHistory();
      if (typeof window.__drawInjGrowth === 'function') window.__drawInjGrowth();
      cloudState.lastSuccess = Date.now();
      setCloudStatus('online', 'CLOUD SYNC');
    }
  }

  async function cloudRequest(address, { push = false, force = false, notify = false } = {}) {
    const wallet = addressKey(address);
    if (!validAddress?.(wallet) || !navigator.onLine || !cloudAvailable()) return null;
    if (cloudState.inFlight && wallet === cloudState.activeAddress) return null;

    cloudState.inFlight = true;
    cloudState.activeAddress = wallet;
    setCloudStatus('loading', 'SYNC…');
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 25000);

    try {
      const response = await fetch(
        push ? CLOUD_ENDPOINT : `${CLOUD_ENDPOINT}?address=${encodeURIComponent(wallet)}${force ? '&sync=1' : ''}`,
        push ? {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(localPayload(wallet)),
          cache: 'no-store',
          signal: controller.signal,
        } : { cache: 'no-store', signal: controller.signal },
      );
      const payload = await response.json().catch(() => ({}));
      if (!response.ok || !payload?.ok) throw new Error(payload?.error || `Cloud HTTP ${response.status}`);
      applyCloudData(payload.data, wallet);
      if (push) localStorage.setItem(cloudPushKey(wallet), String(Date.now()));
      if (notify && typeof toast === 'function') toast('Storici sincronizzati nel cloud');
      return payload.data;
    } catch (error) {
      console.warn('Cloud history sync', error);
      setCloudStatus('error', 'SOLO LOCALE');
      if (notify && typeof toast === 'function') toast('Cloud non disponibile: dati conservati in locale');
      return null;
    } finally {
  ...
  async function syncCurrentWallet({ force = false, notify = false } = {}) {
    const address = addressKey();
    if (!validAddress?.(address)) return;
    const lastPush = Number(localStorage.getItem(cloudPushKey(address)) || 0);
    const shouldPush = force || Date.now() - lastPush > 5 * 60 * 1000;
    return cloudRequest(address, { push: shouldPush, force, notify });
  }

  function scheduleCloudSync(delay = 350) {
    clearTimeout(cloudState.timer);
    cloudState.timer = setTimeout(() => syncCurrentWallet({ force: true }), delay);
  }


  async function registerSavedWallets() {
    const wallets = [...new Set((state?.wallets || []).map(addressKey).filter(address => validAddress?.(address)))].slice(0, 20);
    for (const wallet of wallets) {
      await cloudRequest(wallet, { push: true, force: true });
    }
  }

  window.__syncCloudHistory = syncCurrentWallet;

  document.addEventListener('DOMContentLoaded', () => {
    setCloudStatus('loading', 'CLOUD…');
    setTimeout(() => registerSavedWallets(), 1200);
    document.getElementById('growthSnapshotNow')?.addEventListener('click', () => syncCurrentWallet({ force: true, notify: true }));
    document.getElementById('cloudSyncNow')?.addEventListener('click', () => syncCurrentWallet({ force: true, notify: true }));
    document.addEventListener('inj:wallet-changed', () => scheduleCloudSync(450));
    document.addEventListener('visibilitychange', () => { if (!document.hidden) scheduleCloudSync(250); });
    window.addEventListener('focus', () => scheduleCloudSync(250));
    window.addEventListener('online', () => scheduleCloudSync(250));
    setInterval(() => syncCurrentWallet(), 30000);
  });
})();
