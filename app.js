...
function animateRollingValue(element, previousText, nextText, movement, tone) {
  ...
  if (canRollDigits(previousText, nextText)) {
    for (let i = 0; i < nextText.length; i++) {
      ...
      first.className = 'roll-face roll-old';
      first.textContent = oldChar;
      const second = document.createElement('span');
      second.className = 'roll-face roll-new';
      second.textContent = newChar;

      if (movement === 'up') track.append(first, second);
      else track.append(second, first);

      cell.appendChild(track);
      host.appendChild(cell);
    }
  } else {
    host.classList.add(`roll-whole roll-${movement} tone-${tone}`);
    const size = document.createElement('span');
    size.className = 'roll-size';
    size.textContent = nextText;
    const oldLayer = document.createElement('span');
    oldLayer.className = 'roll-old';
    oldLayer.textContent = previousText;
    const newLayer = document.createElement('span');
    newLayer.className = 'roll-new';
    newLayer.textContent = nextText;
    host.append(size, oldLayer, newLayer);
  }

  element.replaceChildren(host);
  requestAnimationFrame(() => host.classList.add('roll-start'));
  element._rollTimer = setTimeout(() => {
    if (element.dataset.rollToken !== token || element.dataset.renderedValue !== nextText) return;
    element.classList.remove('number-rolling');
    element.textContent = nextText;
  }, 980);
}

function signalChange(element, previous, next, previousText, nextText, { cardFlash = true, positiveWhen = 'up', minInterval = 420 } = {}) {
  ...
  const movement = next > previous ? 'up' : 'down';
  ...
  if (now - number(element.dataset.lastValueAnimation) >= minInterval) {
    element.dataset.lastValueAnimation = String(now);
    animateRollingValue(element, previousText, nextText, movement, tone);
  }

  if (cardFlash && card && now - number(card.dataset.lastFlash) > 720) {
...
function setValue(id, text, numericValue, options = {}) {
  ...
  const minInterval = number(options.minInterval) || 420;
  ...
  if (animate && !state.suppressEffects && visibleChange && numericChange) {
    signalChange(element, previous, next, previousText, nextText, { cardFlash, positiveWhen, minInterval });
  } else {
    clearTimeout(element._rollTimer);
    element.classList.remove('number-rolling');
    element.textContent = nextText;
