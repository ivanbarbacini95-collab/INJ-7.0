...
function renderMarket() {
  ...
  setValue('headerMarketPrice', state.price > 0 ? money(state.price, 3) : '—', state.price || NaN, { flash: false });
...
function renderPortfolio() {
  ...
  setValue('hourlyUsd', state.personalApr > 0 ? money(hourlyInj * state.price, 4) : '—', state.personalApr > 0 ? hourlyInj * state.price : NaN, { flash: false });
  ...
  setValue('weeklyUsd', state.personalApr > 0 ? money(weeklyInj * state.price, 3) : '—', state.personalApr > 0 ? weeklyInj * state.price : NaN, { flash: false });
...
function renderRewardTracker() {
  ...
  setValue('hourlyUsd', active ? money(hourly * state.price, 4) : '—', active ? hourly * state.price : NaN, { flash: false });
  setValue('weeklyEstimate', active ? formatInj(weekly, 6) : '—', active ? weekly : NaN, { flash: false });
  setValue('weeklyUsd', active ? money(weekly * state.price, 3) : '—', active ? weekly * state.price : NaN, { flash: false });
...
function renderTarget() {
  ...
  $('targetPercentBadge').textContent = `${percent.toFixed(1)}%`;
