...
(() => {
  ...
  function updateSelect(){
    ...
    const previous=select.value||localStorage.getItem('inj_live_display_wallet_v777')||'all';
    const rows=wallets();
    select.innerHTML=`<option value="all">Tutti i wallet · separati</option>`+rows.map(a=>`<option value="${key(a)}">${short(a)}</option>`).join('');
    select.value=previous==='all'||rows.some(a=>key(a)===previous)?previous:'all';
  }
  function render(){
    if(!open)return;
    ...
    const visible=chosen==='all'?all:all.filter(w=>key(w.address)===chosen);
    grid.dataset.mode=chosen==='all'?'all':'single';
    $id('landscapeWalletLabel').textContent=chosen==='all'?`TUTTI I WALLET · ${visible.length} SEPARATI`:'WALLET SELEZIONATO';
  ...
  function init(){
    ...
    document.addEventListener('inj:wallet-changed',event=>{
      ...
      if(selected&&selected.value!=='all'&&activeKey){
