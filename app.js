...
const THEME_ORDER=['dark','light','ocean'];
...
function applyTheme(theme,{persist=true}={}){
  const next=THEME_ORDER.includes(theme)?theme:'dark';
  ...
  if(meta) meta.setAttribute('content',next==='light'?'#eef3f8':next==='ocean'?'#06131c':'#05070b');
  if(persist) storage.set('inj_theme',next);
...
function cycleTheme(){
  ...
  applyTheme(next);
  if(typeof syncInterfaceModes==='function') syncInterfaceModes();
  requestAnimationFrame(()=>{ drawAll(); window.dispatchEvent(new Event('resize')); });
  toast(`Tema ${next==='dark'?'Scuro':next==='light'?'Chiaro':'Ocean'}`);
  return next;
}
const storage = {
...
const ROLLING_IDS=new Set([
  'priceUsd','marketPrice','priceChange','dayLow','dayHigh','marketChange',
  ...
  'ohlcOpen','ohlcHigh','ohlcLow','ohlcClose','ohlcVolume','circulatingSupplyValue','liveTotalSupply','networkTotalStaked','networkStakedPct',
  'networkInjBurned','networkBurnedPct','walletStakedShare','walletSupplyShare','validatorCount','rewardPerSecond','growthCurrent','growthDelta','growthCompoundCount',
  'rewardChartTotal','sentimentScore','marketCapRank','simCurrentPrice','simCurrentMarketCap','simPriceMultiple','simPotentialPct','simulatedInjPrice',
  'simulatedWalletInj','simulatedWalletCurrentUsd','simulatedWalletUsd','simulatedWalletGain','simMarketCapFormatted',
  'athValue','milestoneGoal','milestonePercent','milestoneRemaining','oneInjEta','stakeAllocation','liquidAllocationText',
  'landscapeMarketPrice','landscapeMarketChange','landscapeNetWorthUsd','landscapeNetWorthInj'
]);
...
function setText(id,value){
  ...
  rollElement(el,next,numericFromText(next));
}
...
function rollValue(id,formatted,numericValue){
  const el=$(id); if(el) rollElement(el,formatted,numericValue);
}
function rollElement(el,formatted,numericValue){
  if(!el) return;
  ...
  const canCompare=previousText&&!previousText.includes('—')&&Number.isFinite(previousValue)&&Number.isFinite(nextValue);
  const direction=canCompare&&nextValue<previousValue?'down':'up';
  const sameShape=previousText.length===next.length;
  el.classList.add('rolling-number','airport-rolling');
  el.classList.remove('roll-active');
  el.innerHTML='';
  for(let i=0;i<next.length;i++){
    const oldChar=sameShape?(previousText[i]??''):'';
    const newChar=next[i];
    const isDigit=/\d/.test(newChar);
    const changed=canCompare&&sameShape&&isDigit&&/\d/.test(oldChar)&&oldChar!==newChar;
    if(changed){
      ...
      slot.append(oldNode,newNode);el.appendChild(slot);
    }else{
      const span=document.createElement('span');
      span.className=isDigit?'roll-char roll-digit':'roll-char roll-symbol';
      span.textContent=newChar;el.appendChild(span);
    }
  }
  el.dataset.rollText=next;
  if(Number.isFinite(nextValue))el.dataset.rollValue=String(nextValue);
  if(canCompare&&sameShape){
  ...
  }else renderStableNumber(el,next);
}
...
const WALLET_CYCLE_MAX=120;
function walletCycleKey(address){return `inj_wallet_cycle_24h_v2_${String(address||'preview').toLowerCase()}`}
function readWalletCycle(){
  const fallback={baselineUsd:0,baselineInj:0,startedAt:0,lastCheckAt:0,history:[]};
  if(!state.address) return fallback;
  const saved=storage.getJSON(walletCycleKey(state.address),fallback);
  return {...fallback,...saved,history:Array.isArray(saved?.history)?saved.history:[]};
}
function drawWalletCycleChart(){
  const canvas=$('walletCycleChart'),scroll=$('walletCycleScroll'); if(!canvas||!scroll) return;
  const cycle=readWalletCycle(), rows=(cycle.history||[]).filter(x=>Number.isFinite(number(x?.pct)));
  const cssW=Math.max(scroll.clientWidth,Math.max(760,rows.length*58+90));
  const cssH=Math.max(210,canvas.clientHeight||255),dpr=Math.min(2,window.devicePixelRatio||1);
  canvas.style.width=`${cssW}px`;canvas.width=Math.round(cssW*dpr);canvas.height=Math.round(cssH*dpr);
  const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,cssW,cssH);
  const styles=getComputedStyle(document.body),muted=styles.getPropertyValue('--muted').trim()||'#8d9aaf',border=styles.getPropertyValue('--border').trim()||'rgba(255,255,255,.09)',gain=styles.getPropertyValue('--accent').trim()||'#22d3a6',loss=styles.getPropertyValue('--red').trim()||'#ff5d6c';
  const pad={l:48,r:22,t:24,b:42},zero=(pad.t+cssH-pad.b)/2,maxAbs=Math.max(1,...rows.map(x=>Math.abs(number(x.pct))));
  ctx.strokeStyle=border;ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(pad.l,zero+.5);ctx.lineTo(cssW-pad.r,zero+.5);ctx.stroke();
  ctx.fillStyle=muted;ctx.font='11px system-ui';ctx.textAlign='right';ctx.fillText('0%',pad.l-8,zero+4);ctx.fillText(`+${maxAbs.toFixed(1)}%`,pad.l-8,pad.t+4);ctx.fillText(`-${maxAbs.toFixed(1)}%`,pad.l-8,cssH-pad.b+4);
  if(!rows.length){ctx.textAlign='center';ctx.fillText('La prima candela verrà registrata dopo 24 ore',cssW/2,zero-12);return;}
  const plotH=(cssH-pad.t-pad.b)/2-10,slot=(cssW-pad.l-pad.r)/Math.max(rows.length,1),barW=Math.max(8,Math.min(24,slot*.46));
  rows.forEach((row,i)=>{const pct=number(row.pct),x=pad.l+slot*i+slot/2,h=Math.max(2,Math.abs(pct)/maxAbs*plotH),y=pct>=0?zero-h:zero;ctx.fillStyle=pct>=0?gain:loss;ctx.fillRect(x-barW/2,y,barW,h);ctx.fillStyle=muted;ctx.font='10px system-ui';ctx.textAlign='center';const d=new Date(row.t);ctx.fillText(d.toLocaleDateString('it-IT',{day:'2-digit',month:'2-digit'}),x,cssH-22);ctx.fillText(d.toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'}),x,cssH-9);});
}
function renderWalletCycle(total,nw){
  if(!state.address||!total||!state.price){setText('walletCycleStatus','IN ATTESA');setText('walletCycleCountdown','24h 00m');setText('walletCycleLastCheck','Carica un wallet');drawWalletCycleChart();return;}
  const now=Date.now(),cycle=readWalletCycle();
  ...
  if(now-cycle.startedAt>=WALLET_CYCLE_MS){
    const baseline=number(cycle.baselineUsd),deltaUsd=nw-baseline,pct=baseline?deltaUsd/baseline*100:0;
    cycle.history.push({t:now,pct,deltaUsd,deltaInj:total-number(cycle.baselineInj),openUsd:baseline,closeUsd:nw});
    cycle.history=cycle.history.slice(-WALLET_CYCLE_MAX);cycle.lastCheckAt=now;cycle.startedAt=now;cycle.baselineUsd=nw;cycle.baselineInj=total;storage.setJSON(walletCycleKey(state.address),cycle);
    requestAnimationFrame(()=>{drawWalletCycleChart();const sc=$('walletCycleScroll');if(sc)sc.scrollLeft=sc.scrollWidth;});
  }
  const left=Math.max(0,WALLET_CYCLE_MS-(now-cycle.startedAt)),hours=Math.floor(left/3600000),mins=Math.floor((left%3600000)/60000);
  setText('walletCycleCountdown',`${hours}h ${String(mins).padStart(2,'0')}m`);
  setText('walletCycleLastCheck',cycle.lastCheckAt?`Ultima candela: ${new Date(cycle.lastCheckAt).toLocaleString('it-IT')}`:`Baseline: ${new Date(cycle.startedAt).toLocaleString('it-IT')}`);
  setText('walletCycleStatus',cycle.lastCheckAt?'STORICO ATTIVO':'MONITORAGGIO');drawWalletCycleChart();
}
...
function init(){
  ...
  applyTheme(storage.get('inj_theme','dark'),{persist:false});
...
(()=>{
  ...
  function syncInterfaceModes(){
    ...
    Object.entries(modeButtons).forEach(([key,button])=>{
      ...
      if(key==='theme'){ const theme=currentTheme(); button.querySelector('.interface-icon').textContent=theme==='light'?'☀':theme==='ocean'?'≈':'☾'; const label=button.querySelector('span:last-child'); if(label) label.textContent=theme==='dark'?'Scuro':theme==='light'?'Chiaro':'Ocean'; button.title='Tema attuale: '+label.textContent+' · clicca per cambiare'; }
  ...
  modeButtons.theme?.addEventListener('click',cycleTheme);
  ...
  applyTheme(localStorage.getItem('inj_theme')||'dark',{persist:false});
  document.body.classList.toggle('reduce-motion',!motionEnabled);
  syncInterfaceModes();
})();
...
/* v7.7.5 — stable card layout editor with dedicated mouse/touch handles */
(()=>{
  const STORAGE_KEY='inj_card_layout_v775';
  const editableContainers=[];
  let editMode=false, drag=null, raf=0;
  ...
  function containerKey(container,index){
    ...
    const classes=[...container.classList].filter(c=>c!=='active');
    return `zone-${safeId(classes.join('-')||container.tagName.toLowerCase())}-${index}`;
  }
  function directCards(container){return [...container.children].filter(el=>el.matches?.('article.card'));}
  function ensureHandle(card){
    let handle=card.querySelector(':scope > .layout-drag-handle');
    if(!handle){
      handle=document.createElement('button');
      handle.type='button';handle.className='layout-drag-handle';
      handle.setAttribute('aria-label','Trascina per spostare questa card');
      handle.innerHTML='<span aria-hidden="true">⋮⋮</span><b>TRASCINA</b>';
      card.appendChild(handle);
    }
  }
  function discover(){
    editableContainers.length=0;
    const candidates=[document.getElementById('dashboardView'),...document.querySelectorAll('#dashboardView section,#dashboardView .metric-grid,#dashboardView .insight-grid,#dashboardView .pro-grid')].filter(Boolean);
    [...new Set(candidates)].forEach((container,index)=>{
      const cards=directCards(container);if(cards.length<2)return;
      const key=containerKey(container,index);container.dataset.layoutContainer=key;
      cards.forEach((card,i)=>{
        if(!card.dataset.layoutItem){
          const semantic=card.id||card.querySelector('.chart-title')?.textContent||card.querySelector('.eyebrow')?.textContent||card.querySelector('span')?.textContent||`card-${i}`;
          card.dataset.layoutItem=`${key}-${safeId(semantic).slice(0,64)}`;
        }
        ensureHandle(card);
      });
      editableContainers.push(container);
    });
  }
  function readLayout(){try{return JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}')}catch{return {}}}
  function saveLayout(){
    const layout={};editableContainers.forEach(c=>layout[c.dataset.layoutContainer]=directCards(c).map(x=>x.dataset.layoutItem));
    try{localStorage.setItem(STORAGE_KEY,JSON.stringify(layout))}catch{}
  }
  function restoreLayout(){
    discover();const layout=readLayout();
    editableContainers.forEach(container=>{
      const order=layout[container.dataset.layoutContainer];if(!Array.isArray(order))return;
      const map=new Map(directCards(container).map(card=>[card.dataset.layoutItem,card]));
      order.forEach(id=>{const card=map.get(id);if(card)container.appendChild(card)});
    });
  }
  function banner(show){
    let el=document.getElementById('layoutEditBanner');
    if(show&&!el){
      el=document.createElement('div');el.id='layoutEditBanner';el.className='layout-edit-banner';
      el.innerHTML='<span>Usa la maniglia ⋮⋮ per spostare le card</span><button type="button">Fatto</button>';
      el.querySelector('button').addEventListener('click',()=>setEditMode(false));document.body.appendChild(el);
    }
    if(el)el.hidden=!show;
  }
  function syncButton(){
    const b=document.getElementById('drawerLayoutEdit');if(!b)return;
    b.classList.toggle('active',editMode);b.setAttribute('aria-pressed',editMode?'true':'false');
    const label=b.querySelector('span:last-child');if(label)label.textContent=editMode?'Termina modifica':'Modifica layout';
  }
  function setEditMode(on){
    editMode=Boolean(on);discover();document.body.classList.toggle('layout-edit-mode',editMode);banner(editMode);syncButton();
    if(!editMode){cancelDrag();saveLayout();window.dispatchEvent(new Event('resize'));}
  }
  function cancelDrag(){
    cancelAnimationFrame(raf);raf=0;if(!drag)return;
    drag.card.classList.remove('layout-dragging');drag.ghost?.remove();drag.placeholder?.replaceWith(drag.card);drag=null;
  }
  function beginDrag(card,e){
    const rect=card.getBoundingClientRect();
    const placeholder=document.createElement('div');placeholder.className='layout-drop-placeholder';
    placeholder.style.height=`${rect.height}px`;placeholder.style.gridColumn=getComputedStyle(card).gridColumn;
    card.after(placeholder);
    const ghost=card.cloneNode(true);ghost.removeAttribute('id');ghost.querySelectorAll('[id]').forEach(el=>el.removeAttribute('id'));
    ghost.classList.add('layout-drag-ghost');Object.assign(ghost.style,{width:`${rect.width}px`,height:`${rect.height}px`,left:`${rect.left}px`,top:`${rect.top}px`});
    document.body.appendChild(ghost);card.classList.add('layout-dragging');
    drag={card,ghost,placeholder,container:card.parentElement,offsetX:e.clientX-rect.left,offsetY:e.clientY-rect.top,x:e.clientX,y:e.clientY};
  }
  function autoScroll(y){
    const edge=90,speed=14;
    if(y<edge)window.scrollBy(0,-speed);else if(y>innerHeight-edge)window.scrollBy(0,speed);
  }
  function paintDrag(){
    raf=0;if(!drag)return;
    drag.ghost.style.left=`${drag.x-drag.offsetX}px`;drag.ghost.style.top=`${drag.y-drag.offsetY}px`;autoScroll(drag.y);
    const target=document.elementFromPoint(drag.x,drag.y)?.closest?.('article.card[data-layout-item]');
    if(!target||target===drag.card||target.parentElement!==drag.container)return;
    const r=target.getBoundingClientRect();
    const horizontal=Math.abs(drag.y-(r.top+r.height/2))<r.height*.34;
    const before=horizontal?drag.x<r.left+r.width/2:drag.y<r.top+r.height/2;
    drag.container.insertBefore(drag.placeholder,before?target:target.nextSibling);
  }
  function moveDrag(e){if(!drag)return;drag.x=e.clientX;drag.y=e.clientY;if(!raf)raf=requestAnimationFrame(paintDrag);}
  function endDrag(){
    cancelAnimationFrame(raf);raf=0;if(!drag)return;
    drag.placeholder.replaceWith(drag.card);drag.card.classList.remove('layout-dragging');drag.ghost.remove();drag=null;saveLayout();window.dispatchEvent(new Event('resize'));
  }
  function bind(){
    restoreLayout();
    document.getElementById('drawerLayoutEdit')?.addEventListener('click',()=>setEditMode(!editMode));
    document.getElementById('drawerLayoutReset')?.addEventListener('click',()=>{try{localStorage.removeItem(STORAGE_KEY);localStorage.removeItem('inj_card_layout_v772')}catch{}location.reload();});
    document.addEventListener('pointerdown',e=>{
      if(!editMode||e.button>0)return;
      const handle=e.target.closest?.('.layout-drag-handle');if(!handle)return;
      const card=handle.closest('article.card[data-layout-item]');if(!card)return;
      e.preventDefault();handle.setPointerCapture?.(e.pointerId);beginDrag(card,e);
    },{passive:false});
    ...
    window.addEventListener('keydown',e=>{if(e.key==='Escape'&&editMode)setEditMode(false)});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();
})();

/* v7.7.5 — Live Display with automatic multi-wallet aggregation */
(()=>{
  let active=false,syncTimer=null,observer=null;
  const $id=id=>document.getElementById(id);
  const sourceIds=['marketPrice','marketChange','netWorthUsd','netWorthInj','walletTabs'];
  function copyText(source,target){const s=$id(source),t=$id(target);if(t){const text=s?.dataset?.rollText||s?.textContent?.trim()||'—';rollElement(t,text,numericFromText(text));}}
  function formatUsd(value){return new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(value)||0);}
  function formatInj(value){
    const n=Number(value)||0,d=n>=10000?2:n>=1000?3:4;
    return `${new Intl.NumberFormat('en-US',{minimumFractionDigits:d,maximumFractionDigits:d}).format(n)} INJ`;
  }
  function aggregateWallets(){
    const unique=[...new Set((state?.wallets||[]).filter(validAddress).map(a=>String(a).trim().toLowerCase()))];
    const activeKey=String(state?.address||'').trim().toLowerCase();
    if(validAddress?.(state?.address)&&!unique.includes(activeKey))unique.unshift(activeKey);
    if(unique.length<=1)return null;
    let total=0,loaded=0;
    unique.forEach(key=>{
      if(key===activeKey){total+=number(state.available)+number(state.staked)+number(state.rewards);loaded++;return;}
      const cached=state.walletCache?.[key];
      if(cached&&Number.isFinite(Number(cached.total))){total+=Number(cached.total);loaded++;}
    });
    return {count:unique.length,loaded,total};
  }
  function syncValues(){
    copyText('marketPrice','landscapeMarketPrice');copyText('marketChange','landscapeMarketChange');
    const aggregate=aggregateWallets(),label=$id('landscapeWalletLabel');
    if(aggregate){
      const usd=aggregate.total*(number(state.price)||0);
      const usdEl=$id('landscapeNetWorthUsd'),injEl=$id('landscapeNetWorthInj');
      if(usdEl)rollElement(usdEl,formatUsd(usd),usd);if(injEl)rollElement(injEl,formatInj(aggregate.total),aggregate.total);
      if(label)label.textContent=aggregate.loaded===aggregate.count?`CONTROVALORE TOTALE · ${aggregate.count} WALLET`:`CONTROVALORE TOTALE · ${aggregate.loaded}/${aggregate.count} WALLET`;
    }else{
      copyText('netWorthUsd','landscapeNetWorthUsd');copyText('netWorthInj','landscapeNetWorthInj');
      if(label)label.textContent='CONTROVALORE WALLET';
    }
    const src=$id('marketChange'),dst=$id('landscapeMarketChange');
    if(src&&dst){dst.className='landscape-change '+[...src.classList].filter(x=>x!=='market-change').join(' ');dst.style.color=getComputedStyle(src).color;}
  }
  async function enter(){
    active=true;document.body.classList.add('landscape-live-active');
    const panel=$id('landscapeLive');if(panel){panel.hidden=false;panel.setAttribute('aria-hidden','false');}
    const btn=$id('drawerLandscapeLive');btn?.classList.add('active');btn?.setAttribute('aria-pressed','true');$id('drawerClose')?.click();syncValues();
    syncTimer=setInterval(syncValues,250);
    observer=new MutationObserver(syncValues);sourceIds.forEach(id=>{const el=$id(id);if(el)observer.observe(el,{childList:true,characterData:true,subtree:true,attributes:true});});
    try{await document.documentElement.requestFullscreen?.();}catch{}
    if(window.matchMedia('(max-width: 900px), (pointer: coarse)').matches){try{await screen.orientation?.lock?.('landscape');}catch{}}
  }
  async function exit(){
    active=false;document.body.classList.remove('landscape-live-active');
    const panel=$id('landscapeLive');if(panel){panel.hidden=true;panel.setAttribute('aria-hidden','true');}
    const btn=$id('drawerLandscapeLive');btn?.classList.remove('active');btn?.setAttribute('aria-pressed','false');
    clearInterval(syncTimer);syncTimer=null;observer?.disconnect();observer=null;
    try{screen.orientation?.unlock?.();}catch{}try{if(document.fullscreenElement)await document.exitFullscreen?.();}catch{}
  }
  function bind(){
    $id('drawerLandscapeLive')?.addEventListener('click',()=>active?exit():enter());$id('landscapeLiveExit')?.addEventListener('click',exit);
    document.addEventListener('inj:wallet-changed',()=>active&&syncValues());
    document.addEventListener('keydown',e=>{if(active&&e.key==='Escape')exit();});document.addEventListener('fullscreenchange',()=>{if(active&&!document.fullscreenElement)syncValues();});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();
})();
