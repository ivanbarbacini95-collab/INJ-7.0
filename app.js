...
// v7.6.5: storico reward account-first, paginazione after e diagnostica parser.
...
async function fetchRewardHistoryFromExplorer({offset=0,maxPages=50,onProgress}={}){
  const pageSize=100,parsed=[];
  let scanned=0,total=0,after=offset?String(offset):'';
  const seenCursors=new Set();
  for(let page=0;page<maxPages;page++){
    const params=new URLSearchParams({limit:String(pageSize)});
    // L'Explorer REST usa `after`; non applichiamo filtri type/status lato server,
    // perché alcune release dell'Indexer restituiscono zero righe con quei filtri.
    if(after) params.set('after',after);
    const payload=await explorerJson(`/accountTxs/${encodeURIComponent(state.address)}?${params}`);
    const txs=explorerTxList(payload);
    const paging=explorerPaging(payload);
    total=paging.total||total;
    for(const tx of txs) parsed.push(...parseExplorerWithdrawalTx(tx,state.address));
    scanned+=txs.length;
    onProgress?.({skip:scanned,total,scanned,found:parsed.length});
...
async function syncRewardHistoryForAddress(address){
  if(!CHAIN_HISTORY_FEATURES) return [];
  const wallet=String(address||'').trim();
  ...
  try{
    ...
    console.warn('Reward sync failed for wallet',wallet,error);
    return 0;
  }
}
async function syncRewardHistoryForAllWallets(){
  if(!CHAIN_HISTORY_FEATURES) return [];
  const wallets=[...new Set((state.wallets||[]).filter(validAddress).map(address=>String(address).trim()))];
  for(const wallet of wallets) await syncRewardHistoryForAddress(wallet);
}
...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    const syncRewardHistoryAfterOffline=()=>{
      ...
      if(CHAIN_HISTORY_FEATURES) setTimeout(syncRewardHistoryForAllWallets,250);
    };
...
(()=>{
  ...
  function alertValue(a){if(a.metric==='priceAbove'||a.metric==='priceBelow')return num(state.price);if(a.metric==='aprBelow')return num(state.apr);if(a.metric==='rewardAbove')return num(state.rewards);if(a.metric==='networthAbove')return netWorth();return 0}
  function alertTriggered(a){const v=alertValue(a);return a.metric.endsWith('Above')?v>=a.threshold:v<=a.threshold}
  function renderAlerts(check=false){const list=document.getElementById('alertList');if(!list)return;list.innerHTML=alerts.length?alerts.map((a,i)=>{const hit=alertTriggered(a);if(check&&hit&&!a.notified){a.notified=true;toastV7(`Alert raggiunto: ${a.label}`)}return `<div class="alert-row-pro"><div class="alert-row-head"><strong>${a.label}</strong><button class="row-delete" data-alert-delete="${i}">×</button></div><small>${hit?'● RAGGIUNTO':'○ In attesa'} · valore ${alertValue(a).toFixed(2)}</small></div>`}).join(''):'<p class="diagnostic-output">Nessun alert configurato.</p>';write('alerts',alerts)}
  ...
  function bind(){
    ...
    document.getElementById('addAlert')?.addEventListener('click',()=>{const metric=document.getElementById('alertMetric').value,threshold=num(document.getElementById('alertThreshold').value);if(!threshold)return toastV7('Inserisci una soglia');const names={priceAbove:'Prezzo sopra',priceBelow:'Prezzo sotto',aprBelow:'APR sotto',rewardAbove:'Reward sopra',networthAbove:'Net Worth sopra'};alerts.push({metric,threshold,label:`${names[metric]} ${threshold}`,notified:false});write('alerts',alerts);renderAlerts()});
...
/* v7.6.2 — Chain Edition: history reconstructed from Injective transactions */
(() => {
  const GROWTH_PREFIX = 'inj_growth_v733_';
  const CHAIN_SYNC_PREFIX = 'inj_chain_sync_v761_';
  const chainState = { inFlight:false, timer:0, activeAddress:'', lastSuccess:0 };
  const addressKey = (address = state?.address) => String(address || '').trim().toLowerCase();
  const growthStorageKey = (address = state?.address) => `${GROWTH_PREFIX}${addressKey(address)}`;
  const syncKey = (address = state?.address) => `${CHAIN_SYNC_PREFIX}${addressKey(address)}`;

  function setChainStatus(mode,text){
    for(const id of ['cloudSyncStatus','historyCloudStatus']){
      const badge=document.getElementById(id); if(!badge) continue;
      badge.textContent=text; badge.dataset.mode=mode;
  ...
  function coinAmount(value){
    ...
    if(typeof value==='object') return fromWei(value.amount||0);
    const text=String(value); const match=text.match(/([0-9]+(?:\.[0-9]+)?)inj\b/);
    return match?fromWei(match[1]):fromWei(text);
  }
  function flattenMessages(messages=[]){
    ...
    for(const message of messages||[]){
      ...
      if(type.endsWith('MsgExec')&&Array.isArray(message?.msgs)) output.push(...flattenMessages(message.msgs));
    }
    return output;
  }
  function stakingChanges(tx,response,address){
    if(Number(response?.code||0)!==0) return [];
    ...
    const rows=[];
    flattenMessages(tx?.body?.messages||[]).forEach((message,index)=>{
      const type=String(message?.['@type']||message?.type_url||'');
      let delta=0,kind='',validator='';
      if(type.endsWith('MsgDelegate')&&message?.delegator_address===address){ delta=coinAmount(message.amount); kind='delegate'; validator=message.validator_address||''; }
      else if(type.endsWith('MsgUndelegate')&&message?.delegator_address===address){ delta=-coinAmount(message.amount); kind='unstake'; validator=message.validator_address||''; }
      else if(type.endsWith('MsgBeginRedelegate')&&message?.delegator_address===address){ delta=0; kind='redelegate'; validator=message.validator_dst_address||message.validator_src_address||''; }
      if(!kind) return;
      rows.push({id:`${hash}:${index}:${kind}`,hash,height,t,timestamp,delta,amount:Math.abs(delta),type:kind,validator,source:'chain'});
    });
    return rows;
  }
  function dedupe(rows){
    const map=new Map();
    (rows||[]).filter(row=>Number(row?.t)>0).forEach(row=>map.set(String(row.id||`${row.hash}:${row.type}:${row.t}`),row));
    return [...map.values()].sort((a,b)=>a.t-b.t);
  }
  function anchorGrowth(changes,currentStaked){
    const ordered=dedupe(changes);
    const net=ordered.reduce((sum,row)=>sum+Number(row.delta||0),0);
    let running=Math.max(0,Number(currentStaked||0)-net);
    const points=[];
    if(ordered.length) points.push({id:`chain-baseline:${ordered[0].t}`,t:Math.max(1,ordered[0].t-1),timestamp:new Date(Math.max(1,ordered[0].t-1)).toISOString(),staked:running,delta:0,amount:0,type:'baseline',source:'chain'});
    for(const row of ordered){ running=Math.max(0,running+Number(row.delta||0)); points.push({...row,staked:running}); }
    if(!points.length) points.push({id:`chain-current:${addressKey()}`,t:Date.now(),timestamp:new Date().toISOString(),staked:Number(currentStaked||0),delta:0,amount:0,type:'baseline',source:'chain'});
    const last=points.at(-1);
    if(Math.abs(Number(last.staked)-Number(currentStaked||0))>0.000001) points.push({id:`chain-anchor:${Date.now()}`,t:Date.now(),timestamp:new Date().toISOString(),staked:Number(currentStaked||0),delta:Number(currentStaked||0)-Number(last.staked),amount:Math.abs(Number(currentStaked||0)-Number(last.staked)),type:'anchor',source:'chain'});
    return points.slice(-10000);
  }
  async function reconstructChainHistory({notify=false}={}){
  if(!CHAIN_HISTORY_FEATURES) return [];
    const wallet=addressKey();
    if(!validAddress?.(wallet)||!navigator.onLine) return null;
    if(chainState.inFlight&&chainState.activeAddress===wallet) return null;
    chainState.inFlight=true; chainState.activeAddress=wallet; setChainStatus('loading','CHAIN…');
    try{
      const pageSize=100; let offset=0,total=0; const changes=[];
      for(let page=0;page<20;page++){
        const event=encodeURIComponent(`message.sender='${wallet}'`);
        const data=await lcd(`/cosmos/tx/v1beta1/txs?events=${event}&pagination.limit=${pageSize}&pagination.offset=${offset}&pagination.count_total=true&order_by=ORDER_BY_ASC`);
        const txs=data?.txs||[], responses=data?.tx_responses||[];
        total=number(data?.pagination?.total)||total;
        for(let i=0;i<txs.length;i++) changes.push(...stakingChanges(txs[i],responses[i]||{},wallet));
        offset+=txs.length;
        if(txs.length<pageSize||(total&&offset>=total)) break;
      }
      const growth=anchorGrowth(changes,state.staked);
      storage.setJSON(growthStorageKey(wallet),growth);
      storage.set(syncKey(wallet),String(Date.now()));
      chainState.lastSuccess=Date.now();
      if(typeof window.__drawInjGrowth==='function') window.__drawInjGrowth();
      if(typeof drawAll==='function') requestAnimationFrame(()=>{ drawAll(); requestAnimationFrame(drawAll); });
      document.dispatchEvent(new CustomEvent('inj:chain-history-ready',{detail:{address:wallet,growthPoints:growth.length}}));
      if(typeof loadRewardHistory==='function'){ state.rewardHistorySyncedSession=false; await loadRewardHistory({showFeedback:false}); }
      setChainStatus('online','ON-CHAIN');
      if(notify&&typeof toast==='function') toast(`Chain aggiornata: ${changes.length} movimenti staking`);
      return growth;
    }catch(error){
      console.warn('Chain history reconstruction',error); setChainStatus('error','CHAIN OFFLINE');
      if(notify&&typeof toast==='function') toast('Chain non raggiungibile: uso la cache locale');
      return null;
    }finally{ chainState.inFlight=false; }
  }
  function schedule(delay=350){ clearTimeout(chainState.timer); chainState.timer=setTimeout(()=>reconstructChainHistory(),delay); }
  window.__syncChainHistory=reconstructChainHistory;
  document.addEventListener('DOMContentLoaded',()=>{
    setChainStatus('loading','CHAIN…');
    document.getElementById('growthSnapshotNow')?.addEventListener('click',()=>reconstructChainHistory({notify:true}));
    document.getElementById('cloudSyncNow')?.addEventListener('click',()=>reconstructChainHistory({notify:true}));
    document.addEventListener('inj:wallet-changed',()=>schedule(500));
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule(250)});
    window.addEventListener('online',()=>schedule(250));
    setTimeout(()=>reconstructChainHistory(),1200);
    setInterval(()=>reconstructChainHistory(),5*60*1000);
  });
})();
