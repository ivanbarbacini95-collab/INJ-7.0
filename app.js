...
function canAnimateValue(previousText, nextText) {
  if (!previousText || previousText === nextText) return false;
  // Keep the layout absolutely stable: animate only when the formatted string
  // has the same length. Length/format changes are applied directly.
  return previousText.length === nextText.length;
}

function ensureValueMotionShell(element, text = '') {
  let shell = element.firstElementChild;
  if (!shell || !shell.classList.contains('value-motion-shell')) {
    shell = document.createElement('span');
    shell.className = 'value-motion-shell';
    const current = document.createElement('span');
    current.className = 'value-motion-current';
    current.textContent = String(text);
    shell.appendChild(current);
    element.replaceChildren(shell);
  }
  let current = shell.querySelector('.value-motion-current');
  if (!current) {
    current = document.createElement('span');
    current.className = 'value-motion-current';
    current.textContent = String(text);
    shell.replaceChildren(current);
  }
  return { shell, current };
}

function renderStableValue(element, text, numericValue) {
  clearTimeout(element._motionDelayTimer);
  if (element._valueAnimation) {
    try { element._valueAnimation.old.cancel(); } catch (_) {}
    try { element._valueAnimation.next.cancel(); } catch (_) {}
    element._valueAnimation = null;
  }
  element.classList.remove('value-is-rolling');
  const { shell, current } = ensureValueMotionShell(element, text);
  shell.querySelectorAll('.value-motion-next').forEach((node) => node.remove());
  current.textContent = String(text);
  current.style.removeProperty('transform');
  current.style.removeProperty('opacity');
  current.style.removeProperty('color');
  current.style.removeProperty('text-shadow');
  element.dataset.displayedValue = String(text);
  if (Number.isFinite(Number(numericValue))) element.dataset.displayedNumeric = String(Number(numericValue));
  else delete element.dataset.displayedNumeric;
}
...
function flashValueCard(element, tone, cardFlash = true) {
  ...
  if (!card) return;
  const now = Date.now();
  if (now - number(card.dataset.lastFlash) <= 720) return;
...
function runValueTransition(element, nextText, nextNumeric, options = {}) {
  const animate = options.animate !== false;
  const cardFlash = options.cardFlash ?? (options.flash !== false);
  const positiveWhen = options.positiveWhen || 'up';
  const duration = Math.max(420, number(options.duration) || 620);

  const previousText = element.dataset.displayedValue ?? element.dataset.renderedValue ?? element.textContent ?? '';
  const previousNumeric = element.dataset.displayedNumeric === undefined ? NaN : Number(element.dataset.displayedNumeric);
  const next = Number(nextNumeric);
  const visibleChange = previousText !== nextText;
  const numericChange = Number.isFinite(previousNumeric) && Number.isFinite(next) &&
    Math.abs(next - previousNumeric) > Math.max(1e-12, Math.abs(previousNumeric) * 1e-10);

  if (!visibleChange) {
    element.dataset.displayedValue = nextText;
    if (Number.isFinite(next)) element.dataset.displayedNumeric = String(next);
    return;
  }

  if (!animate || state.suppressEffects || !numericChange || !canAnimateValue(previousText, nextText)) {
    renderStableValue(element, nextText, next);
    return;
  }
  ...
  const { shell, current } = ensureValueMotionShell(element, previousText);

  // The current span remains the sizing element for the entire transition.
  // The incoming span is absolute, so the card can never reflow or jump.
  const incoming = document.createElement('span');
  incoming.className = `value-motion-next tone-${tone}`;
  ...
  incoming.style.transform = `translate3d(0, ${-distance * 100}%, 0)`;
  incoming.style.opacity = '1';

  element.classList.add('value-is-rolling');
  flashValueCard(element, tone, cardFlash);
  element._lastMotionAt = Date.now();

  const oldAnimation = current.animate([
    { transform: 'translate3d(0,0,0)', opacity: 1 },
    { transform: `translate3d(0,${distance * 100}%,0)`, opacity: .2 }
  ], { duration, easing, fill: 'forwards' });

  const newAnimation = incoming.animate([
    { transform: `translate3d(0,${-distance * 100}%,0)`, opacity: .28 },
    { transform: 'translate3d(0,0,0)', opacity: 1 }
  ], { duration, easing, fill: 'forwards' });

  element._valueAnimation = { old: oldAnimation, next: newAnimation };

  Promise.allSettled([oldAnimation.finished, newAnimation.finished]).then(() => {
    if (!element._valueAnimation || element._valueAnimation.next !== newAnimation) return;

    // Commit the already-visible incoming value in the exact same shell.
    // Because the shell never leaves the DOM, there is no end-of-animation snap.
    current.textContent = nextText;
    current.style.removeProperty('transform');
    current.style.removeProperty('opacity');
    incoming.remove();
    try { oldAnimation.cancel(); } catch (_) {}
    try { newAnimation.cancel(); } catch (_) {}
    element._valueAnimation = null;
    element.classList.remove('value-is-rolling');
    element.dataset.displayedValue = nextText;
    if (Number.isFinite(next)) element.dataset.displayedNumeric = String(next);
    ...
    const pending = element._pendingValue;
    element._pendingValue = null;
    if (pending && pending.text !== nextText) {
      requestAnimationFrame(() => runValueTransition(element, pending.text, pending.numeric, pending.options));
    }
  });
}
...
function setValue(id, text, numericValue, options = {}) {
  ...
  const latestText = element.dataset.renderedValue;
  const latestNumeric = element.dataset.numericValue === undefined ? NaN : Number(element.dataset.numericValue);
  const sameTarget = latestText === nextText &&
    ((!Number.isFinite(next) && !Number.isFinite(latestNumeric)) ||
     (Number.isFinite(next) && Number.isFinite(latestNumeric) && next === latestNumeric));
  ...
  if (element._valueAnimation) {
    element._pendingValue = { text: nextText, numeric: next, options: { ...options } };
    return;
  }

  if (element.dataset.displayedValue === undefined) {
    renderStableValue(element, nextText, next);
    return;
  }

  const minInterval = Math.max(0, number(options.minInterval));
  const elapsed = Date.now() - number(element._lastMotionAt);
  if (minInterval > 0 && elapsed < minInterval) {
    element._pendingValue = { text: nextText, numeric: next, options: { ...options } };
    clearTimeout(element._motionDelayTimer);
    element._motionDelayTimer = setTimeout(() => {
      const pending = element._pendingValue;
      element._pendingValue = null;
      if (pending) runValueTransition(element, pending.text, pending.numeric, pending.options);
    }, Math.max(16, minInterval - elapsed));
    return;
  }

  runValueTransition(element, nextText, next, options);
}
...
function renderMarket() {
  ...
  setValue('headerMarketPrice', state.price > 0 ? money(state.price, 3) : '—', state.price || NaN, { cardFlash: false });
...
function renderPortfolio() {
  ...
  setValue('stakedShare', `${stakedShare.toFixed(1)}%`, stakedShare, { cardFlash: false });
  setValue('liquidShare', `${liquidShare.toFixed(1)}%`, liquidShare, { cardFlash: false });
