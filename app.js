...
function stopDigitRoll(element) {
  if (!element) return;
  clearTimeout(element._digitRollTimer);
  element.querySelector(':scope > .digit-roll-overlay')?.remove();
  element.classList.remove('digit-roll-active', 'digit-roll-host');
  element.style.removeProperty('--digit-roll-base');
  element.style.removeProperty('--digit-roll-tone');
}
...
function animateChangedDigits(element, previousText, nextText, movement, tone, minInterval = 500) {
  ...
  if (!previousChars.length || previousChars.length !== nextChars.length || nextChars.length > 48) return false;

  const changed = [];
  for (let index = 0; index < nextChars.length; index += 1) {
    if (previousChars[index] === nextChars[index]) continue;
    if (!/\d/.test(previousChars[index]) || !/\d/.test(nextChars[index])) return false;
    changed.push(index);
  }
  if (!changed.length || changed.length > 12) return false;

  const now = Date.now();
  if (now - number(element._lastDigitRollAt) < minInterval) return false;
  element._lastDigitRollAt = now;

  stopDigitRoll(element);

  const computed = window.getComputedStyle(element);
  element.style.setProperty('--digit-roll-base', computed.color);
  element.style.setProperty('--digit-roll-tone', tone === 'up' ? 'var(--up)' : 'var(--down)');
  element.classList.add('digit-roll-host', 'digit-roll-active');

  const overlay = document.createElement('span');
  overlay.className = `digit-roll-overlay digit-roll-${movement}`;
  ...
  nextChars.forEach((char, index) => {
    ...
    spacer.className = 'digit-roll-spacer';
    spacer.textContent = nextChars[index];

    const oldDigit = document.createElement('span');
    oldDigit.className = 'digit-roll-old';
    oldDigit.textContent = previousChars[index];

    const newDigit = document.createElement('span');
    newDigit.className = 'digit-roll-new';
    newDigit.textContent = nextChars[index];

    slot.append(spacer, oldDigit, newDigit);
    overlay.appendChild(slot);
  });

  element.appendChild(overlay);
  element._digitRollTimer = setTimeout(() => stopDigitRoll(element), 560);
  return true;
}
...
function setValue(id, text, numericValue, options = {}) {
  ...
  // The real value always remains ordinary text and owns the layout. Any digit
  // rolling is only a temporary absolute overlay, so Safari cannot stretch cards.
  stopDigitRoll(element);
  ...
  const rolled = animateChangedDigits(element, previousText, nextText, movement, tone, minInterval);
